...
### ⚡ Special Feature of `&&` and `||`: Short-circuiting
...
If `a == 0`, then `10 / a` **won’t be evaluated**, avoiding division by zero.
...
if (a & (b++ > 0))  { } // b is incremented, even though a == 0
...
👉 Equivalent to dividing by `2ⁿ` (if `a` is unsigned)
...
unsigned int a = 5;      // 0b00000101

  

int x = a << 1;          // 0b00001010 = 10

int y = a >> 1;          // 0b00000010 = 2
...
    - If `a` is **unsigned**, `>>` fills with zeros on the left (**logical shift**).

    - If `a` is **signed**, behavior depends on the compiler:

        - It may fill with zeros (**logical shift**),

        - Or replicate the sign bit (**arithmetic shift**, preserves the sign).
...
int a = -4;         // typically: 0b11111100 (in 8-bit representation)

int b = a >> 1;     // may become 0b11111110 (-2), preserving the sign

```

  

⚠️ Behavior may vary between platforms!
...
- Fast multiplication/division by `2ⁿ`
